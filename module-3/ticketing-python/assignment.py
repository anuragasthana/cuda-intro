...
def execute_ticketing_system_participation(ticket_number, part_id, shared_variable, tickets):
    ...
    def run(self):
        ...
        if self.test_username_equality(self.USERNAME):
            ...
            for index in range(self.num_threads):
                ...
                thread = Thread(group=None, target=execute_ticketing_system_participation, args=[index, self.part_id, Lock(), self.tickets])
                thread.start()
